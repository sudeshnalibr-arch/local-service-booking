...
export const searchProviders = createAsyncThunk(
  ...
  async ({
    ...
    let availabilityProviders: ServiceProvider[] | null = null;
    let feeProviders: ServiceProvider[] | null = null;
    ...
    /* --------- 1️⃣ AVAILABILITY API (if time selected) --------- */
    if (date) {
      ...
      const allProviders = normalize(res);
      const formattedTime = normalizeTime(date);

      availabilityProviders = allProviders.filter((p) =>
        p.availability?.some(
          (slot) => normalizeTime(slot) === formattedTime
        )
      );
    ...
    if (minFee !== undefined || maxFee !== undefined) {
      ...
      feeProviders = normalize(res);
    }

    /* --------- 3️⃣ COMBINE RESULTS (CORE LOGIC) --------- */
    let finalProviders: ServiceProvider[] = [];

    if (availabilityProviders && feeProviders) {
      const feeIds = new Set(feeProviders.map((p) => p.id));
      finalProviders = availabilityProviders.filter((p) =>
        feeIds.has(p.id)
      );
    }
    else if (availabilityProviders) {
      finalProviders = availabilityProviders;
    }
    else if (feeProviders) {
      finalProviders = feeProviders;
    }

    console.log("Final Combined Providers:", finalProviders);
...
export const { clearSearchMode, resetProviders, nextPage } =
