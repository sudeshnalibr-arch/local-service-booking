...
import axios from "axios";
import { RootState, AppDispatch } from "@/redux/store/store";
import { addToCart, increaseQty, decreaseQty } from "@/redux/slice/checkoutSlice";
import { useRouter } from "next/navigation";
import "@/style/servicedetails/electrician/electriciancontent.css";
...
export default function ElectricianContent() {
  ...
  useEffect(() => {
    const fetchEquipments = async () => {
      try {
        ...
        const data = res.data.equipments || [];
        setEquipments(data);
        if (data.length > 0) setSelectedCards([data[0]]);
      } catch (err) {
        console.log("Error fetching equipments:", err);
      }
    };
  ...
  const handleEquipmentClick = (equipment: Equipment) => {
    const alreadyAdded = selectedCards.find((i) => i.id === equipment.id);
    if (!alreadyAdded) setSelectedCards((prev) => [...prev, equipment]);
  };
          ...
          {selectedCards.map((service) => (
            <div key={service.id} className="service-card">
              <div className="card-info">
                ...
                <div className="price">₹ {service.price} · 30 mins</div>
              </div>
